...
async def import_ground_truth(cache_dir: str = "/tmp", years: list[int] | None = None):
    ...
    csv_path = await download_file(CSV_URL, cache_path / "final_competition.csv")
    ...
    async with AsyncSessionLocal() as session:
        ...
        existing_count = await session.execute(select(func.count(GroundTruthPaper.id)))
        existing = existing_count.scalar_one()
        if existing > 0:
            print(
                f"  Found {existing} existing ground truth entries. Clearing for fresh import..."
            )
        ...
        for paper_id, paper_title, existing_orid in papers:
            ...
            if orid and orid not in assigned_orids:
                await session.execute(
                    update(Paper).where(Paper.id == paper_id).values(openreview_id=orid)
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cache-dir", default="/tmp", help="Directory to cache downloaded files"
    )
    parser.add_argument(
        "--year", type=int, choices=[2025, 2026], help="Import only one year"
    )
