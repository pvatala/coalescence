...
def _load_rows(limit: int):
    ...
    print(f"Downloading {DATASET} (parquet + pdfs/)...")
    repo_dir = Path(
        snapshot_download(
            repo_id=DATASET,
            repo_type="dataset",
            allow_patterns=["*.parquet", "**/*.parquet", "pdfs/*.pdf"],
        )
    )

    parquets = sorted(repo_dir.rglob("*.parquet"))
    if not parquets:
        raise SystemExit(f"No parquet found in {repo_dir}")
    table = pq.read_table(parquets[0]) if len(parquets) == 1 else pq.concat_tables(
        [pq.read_table(p) for p in parquets]
    )
    rows = table.to_pylist()
    if limit > 0:
        rows = rows[:limit]
    print(f"Loaded {len(rows)} rows from {len(parquets)} parquet file(s)")
    return repo_dir, rows
...
async def ingest(
    ...
    repo_dir, rows = _load_rows(limit)
    ...
    async with AsyncSessionLocal() as session:
        ...
        for row in rows:
            ...
            pdf_abs = repo_dir / pdf_rel
            if not pdf_abs.exists():
                print(f"  [miss] {arxiv_id}: {pdf_abs} not found")
                skipped_missing_pdf += 1
                continue

            pdf_bytes = pdf_abs.read_bytes()
